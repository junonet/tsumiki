...
class tsumiki_bread_crumb{

function bread_crumb( $args = '' ) {

	$default = array(
		'type'				=> 'list',
		'home_label'		=> __( 'Home', 'tsumiki' ),
		'search_label'		=> __( 'Search Result of "%s"', 'tsumiki' ),
		'404_label'			=> __( '404 Not Found', 'tsumiki' ),
		'category_label'	=> _x( '%s', 'category label', 'tsumiki' ),
		'tag_label'			=> _x( '%s', 'tag label', 'tsumiki' ),
		'taxonomy_label'	=> _x( '%s', 'taxonomy label', 'tsumiki' ),
		'author_label'		=> _x( '%s', 'author label', 'tsumiki' ),
		'attachment_label'	=> _x( '%s', 'attachment label', 'tsumiki' ),
		'year_label'		=> _x( '%s', 'year label', 'tsumiki' ),
		'month_label'		=> _x( '%s', 'month label', 'tsumiki' ),
		'day_label'			=> _x( '%s', 'day label', 'tsumiki' ),
		'post_type_label'	=> _x( '%s', 'post type label', 'tsumiki' ),
		'joint_string'		=> __( ' &gt; ', 'tsumiki' ),
		'navi_element'		=> '',
	...
	if ( $args['type'] == 'string' ) {
		...
		foreach ( $bread_crumb_arr as $ancestor ) {
			...
			if ( $cnt == count( $bread_crumb_arr ) ) {
				...
				$output .= $elm_tabs . $tabs . '<i class="material-icons md-18">&#xE01F;</i>' . '	<li class="' . implode( ' ', $classes );
				if ( $args['li_class'] ) {
...
$tsumiki_bread_crumb = new tsumiki_bread_crumb();

if ( ! function_exists( 'bread_crumb' ) ) {
function bread_crumb( $args = '' ) {
	global $tsumiki_bread_crumb;
	return $tsumiki_bread_crumb->bread_crumb( $args );
}
}
